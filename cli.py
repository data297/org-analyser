...
repo-quality-score (sealed) for one GitHub org, one GitLab group, or a folder
of downloaded/local repos per run.

Tokens (github-data-token / gitlab_token / openai_key), workers, hosts,
default target, etc. all live in config.yml's `tokens:` mapping and top-level
keys at the repo root — see config.example.yml. CLI flags always override
config.yml. Pass --tokens-file to use a separate key=value file instead.
...
def run_repo_analyzer(ctx: RunContext, log: PipelineLogger) -> dict[str, Any]:
    """Vendor-style 57-column CSV (LoC, tests, CI, LLM attribution, secrets,
    license risk, quality score, ...) via analysis/repo_analyzer.py."""
    log.info("Phase: repo-analyzer report (vendor CSV)")
    ctx.repo_analyzer_dir.mkdir(parents=True, exist_ok=True)
    ...
    output_csv = ctx.repo_analyzer_dir / "repo_report.csv"
    args = ["--name", ctx.target, "--workers", str(ctx.workers), "--output", str(output_csv)]

    if ctx.platform == "github":
        args.extend(["--org", ctx.target, "--token", gh_token])
        if ctx.github_host != "github.com":
            args.extend(["--github-url", f"https://{ctx.github_host}/api/v3"])
    elif ctx.platform == "github-repo":
        args.extend(["--provider", "github", "--token", gh_token])
        for repo in ctx.github_repos:
            args.extend(["--project", repo])
        if ctx.github_host != "github.com":
            args.extend(["--github-url", f"https://{ctx.github_host}/api/v3"])
        if not gh_token:
            return {"ok": False, "error": "github token required for repo-analyzer"}
    elif ctx.platform == "gitlab":
        args.extend(["--group", ctx.target, "--token", gl_token])
        if ctx.gitlab_host != "gitlab.com":
            args.extend(["--gitlab-url", f"https://{ctx.gitlab_host}"])
    elif ctx.platform == "gitlab-project":
        args.extend(["--provider", "gitlab", "--token", gl_token])
        for project in ctx.gitlab_projects:
            args.extend(["--project", project])
        if ctx.gitlab_host != "gitlab.com":
            args.extend(["--gitlab-url", f"https://{ctx.gitlab_host}"])
        if not gl_token:
            return {"ok": False, "error": "gitlab token required for repo-analyzer"}
    else:
        args.extend(["--provider", "local", "--group", str(ctx.local_repos_dir)])

    try:
        proc = subprocess.run(
            [PYTHON_BIN, "-m", "analysis.repo_analyzer", *args],
            capture_output=True,
            text=True,
            timeout=86400,
            env=env,
        ...
        log.phase_log(ctx.logs_dir / "repo-analyzer.log", "timeout after 24h")
        return {"ok": False, "error": "timeout"}

    detail = (proc.stdout or "") + (proc.stderr or "")
    log.phase_log(ctx.logs_dir / "repo-analyzer.log", detail[-50000:])

    if proc.returncode != 0:
        log.error(f"repo-analyzer failed: {detail[-500:]}")
        return {"ok": False, "error": detail[-1000:]}

    result: dict[str, Any] = {"ok": True}
    if output_csv.is_file():
        result["report_csv"] = str(output_csv)
    log.info(f"repo-analyzer complete -> {output_csv}")
    return result
...
def parse_args() -> argparse.Namespace:
    ...
    if not any(
        [args.github_org, args.github_repo, args.gitlab_group, args.gitlab_project, args.local_repos_dir]
    ):
        parser.error(
            "one target required: --github-org, --github-repo, --gitlab-group, "
            "--gitlab-project, or --local-repos-dir (or set one in config.yml)"
        )
    return args
...
def run_pipeline() -> int:
    ...
    try:
        ...
        repo_analyzer_summary = run_repo_analyzer(ctx, log)
        ctx.manifest["phases"]["repo-analyzer"] = repo_analyzer_summary
