...
This tool is one part of the bigger `repo-pipeline` project. Run this in your Terminal:
...
Every command below is run from here (the repo root) — you do **not** need
to `cd` into any subfolder.
...
In your Terminal (still at the repo root), run **one** of these — note the
`.env` file goes inside `repo_pipeline/eval/`, not the repo root:

**GitHub users:**
```
echo "GITHUB_TOKEN=ghp_PASTE_YOUR_TOKEN_HERE" > repo_pipeline/eval/.env
```

**GitLab users:**
```
echo "GITLAB_TOKEN=glpat-PASTE_YOUR_TOKEN_HERE" > repo_pipeline/eval/.env
```
...
Open or create the `repo_pipeline/eval/.env` file in a text editor and add:
...
Before running any evaluation commands, you must first navigate to the repo root and activate the virtual environment:
...
```
repo-evaluator acme-corp/billing-service --json --output results.json
```

### GitLab project

```
repo-evaluator gitlab:acme-corp/billing-service --platform gitlab --json --output results.json
```

### Faster version (skips slower AI-powered checks)

```
repo-evaluator acme-corp/billing-service --json --output results.json --skip-f2p --skip-quality-checks --skip-taxonomy --skip-pr-rubrics
```
...
```
repo-evaluator https://svn.example.com/myproject/trunk --platform svn --json --output results.json --svn-username YOUR_USER --token YOUR_PASSWORD
```

You can prefix the URL with `svn:` if you like — both styles work:

```
repo-evaluator svn:https://svn.example.com/myproject/trunk --platform svn --json --output results.json --svn-username YOUR_USER --token YOUR_PASSWORD
```

**Check out a specific revision** (optional):

```
repo-evaluator https://svn.example.com/myproject/trunk --platform svn --svn-revision 12345 --json --output results.json --svn-username YOUR_USER --token YOUR_PASSWORD
```
...
```
repo-evaluator https://svn.example.com/myproject/trunk --platform svn --repo-path /path/to/your/checkout --json --output results.json
```

**Faster SVN run** (skips slow / PR-oriented steps):

```
repo-evaluator https://svn.example.com/myproject/trunk --platform svn --json --output results.json --skip-f2p --skip-quality-checks --skip-taxonomy --skip-pr-rubrics --svn-username YOUR_USER --token YOUR_PASSWORD
```
...
```
repo-evaluator . --json --output results.json
```

**Absolute path:**

```
repo-evaluator /Users/you/projects/my-app --json --output results.json
```

**Home-relative path:**

```
repo-evaluator ~/projects/my-app --json --output results.json
```

**Explicit platform flag** (handy if auto-detection is ambiguous):

```
repo-evaluator some-folder --platform local --json --output results.json
```

**Faster local run** (skips slow / PR-oriented steps):

```
repo-evaluator /path/to/project --json --output results.json --skip-f2p --skip-quality-checks --skip-taxonomy --skip-pr-rubrics
```
...
**GitHub:**
```
run-all-repos --dry-run
```

**GitLab:**
```
run-all-repos --platform gitlab --dry-run
```
...
**GitLab:**
```
run-all-repos --platform gitlab --run
```
...
Create a file such as `svn_urls.txt` in the repo root. Each non-empty line is one repository. Lines starting with `#` are ignored.
...
```
bulk-svn-evaluator --urls-file svn_urls.txt --workers 4 --output-dir eval_results/svn_bulk --svn-username YOUR_USER --token YOUR_PASSWORD
```

**Faster run** (skips heavy steps for each repo):

```
bulk-svn-evaluator --urls-file svn_urls.txt --workers 4 --output-dir eval_results/svn_bulk --svn-username YOUR_USER --token YOUR_PASSWORD --evaluator-args "--skip-f2p --skip-quality-checks --skip-taxonomy --skip-pr-rubrics"
```

**Optional:** same revision for all lines that omit one:

```
bulk-svn-evaluator --urls-file svn_urls.txt --default-revision 9000 --svn-username YOUR_USER --token YOUR_PASSWORD
```
...
```
run-all-repos --run --org acme-corp
```

### "I also want to include my personal repos"

```
run-all-repos --run --include-user-repos
```

### "I only care about private repos"

```
run-all-repos --run --visibility private
```

### "I want to skip a specific project"

```
run-all-repos --run --exclude-repo acme-corp/old-project
```
...
```
repo-evaluator /path/to/my/project --json --output results.json
```
...
| **"API rate limit exceeded"** | Your token isn't being picked up. Make sure the `.env` file is in the `repo_pipeline/eval/` folder. |
...
| **Commands don't work after I re-open Terminal** | You need to re-activate the tool every time: run `cd repo-pipeline` then `source .venv/bin/activate` |
...
|---|---|
| See all your GitHub repos (preview) | `run-all-repos --dry-run` |
| See all your GitLab repos (preview) | `run-all-repos --platform gitlab --dry-run` |
| Evaluate **all** GitHub repos | `run-all-repos --run` |
| Evaluate **all** GitLab repos | `run-all-repos --platform gitlab --run` |
| Evaluate all repos **(fast mode)** | `run-all-repos --run --evaluator-args "--skip-f2p --skip-quality-checks --skip-taxonomy --skip-pr-rubrics"` |
| Evaluate only one organization | `run-all-repos --run --org my-org-name` |
| Evaluate **one** GitHub project | `repo-evaluator owner/repo --json --output results.json` |
| Evaluate **one** GitLab project | `repo-evaluator gitlab:group/repo --platform gitlab --json --output results.json` |
| Evaluate a **local folder** | `repo-evaluator /path/to/project --json --output results.json` |
| Evaluate **current directory** | `repo-evaluator . --json --output results.json` |
| Evaluate **one** SVN project | `repo-evaluator https://svn.example.com/proj/trunk --platform svn --json --output results.json --svn-username USER --token PASS` |
| Evaluate **one** SVN project at revision *N* | `repo-evaluator URL --platform svn --svn-revision N --json --output results.json --svn-username USER --token PASS` |
| Evaluate **many** SVN URLs from a file | `bulk-svn-evaluator --urls-file svn_urls.txt --workers 4 --svn-username USER --token PASS` |
| Evaluate one project **(fast)** | `repo-evaluator owner/repo --json --output results.json --skip-f2p --skip-quality-checks --skip-taxonomy --skip-pr-rubrics` |
| Combine all CSVs into one spreadsheet | `consolidate-eval-output` |
...
| **Dependencies**: `pip install -e ".[ui]"` | Pulls **`streamlit`**, **`requests`**, **`python-dotenv`**, **`openai`**, etc. from the repo-root `pyproject.toml`. |
| **`.env`** in **`repo_pipeline/eval/`** | Same file as CLI: **`GITHUB_TOKEN`** or **`GH_TOKEN`** for GitHub API. |
...
From the repo root (same place as `pyproject.toml`):

```bash
source .venv/bin/activate    # Windows: .venv\Scripts\activate
pip install -e ".[ui]"
streamlit run repo_pipeline/eval/ui/github_eval_picker.py
```

Alternative:

```bash
python -m streamlit run repo_pipeline/eval/ui/github_eval_picker.py
```
...
```bash
python -m repo_pipeline.eval.cybersecurity_pr_scanner --repo owner/name --token "$GITHUB_TOKEN" --json-out ./code_security_prs.json --layer1-threshold 6
```

Heuristics only (no OpenAI):

```bash
python -m repo_pipeline.eval.cybersecurity_pr_scanner --repo owner/name --token "$GITHUB_TOKEN" --json-out ./code_security_l1.json --skip-layer2
```
