...
## Optional: confidential scoring mode — scores go ONLY in the sealed file

If the user asks for confidential/blind scoring (e.g. you're running this where
the score itself shouldn't be visible in shared chat logs or a shared terminal —
grading someone else's submission, a shared review session, etc.), follow this
mode: **never reveal the scores anywhere a person in that shared context can
...
Outside of this mode, report scores normally in chat — the sealed bundle is
still produced (it's how re-derivation/tamper-detection works, see
`references/verification.md`), it just isn't the only place the numbers appear.
...
6. **Score + seal in one call.** Run `score.py <scores.json> --report <sealed.json>`
   ...
   crippled.
7. **Report completion.** In confidential scoring mode (see above), surface ONLY:
   "scoring complete" + the **absolute path** of the sealed bundle + "download
   this file and send it back" — do **not** state the score, grade, breakdown, or
   any strength/weakness in chat. Otherwise, report the score/grade/breakdown
   normally and still mention the sealed bundle path.
...
4. **Report completion.** In confidential scoring mode (see above), surface ONLY:
   "org scoring complete" + the **absolute path** of the sealed bundle + "download
   this file and send it back" — do **not** state the org score, grade,
   distribution, class mix, rankings, or any per-repo number in chat. Otherwise,
   report the results normally and still mention the sealed bundle path.
